...
async function testImage(graphModel, imgPath) {
    ...
    // The model input expects Float32 normalized to [-1, 1] or [0, 1] or raw?
    // Let's print output for all three normalization types.
    ...
    const maxValRaw = probRaw.max(1).dataSync()[0];
    ...
    const maxValB = probB.max(1).dataSync()[0];
    ...
    const maxValC = probC.max(1).dataSync()[0];
    const val392C = probC.dataSync()[392];

    console.log(`Image: ${path.basename(imgPath)}`);
    console.log(`  Raw [0-255]: top_idx=${maxIdxRaw} top_val=${maxValRaw.toFixed(4)} class392=${val392Raw.toFixed(4)}`);
    console.log(`  Div [0-1]  : top_idx=${maxIdxB} top_val=${maxValB.toFixed(4)} class392=${val392B.toFixed(4)}`);
    console.log(`  Norm[-1,1] : top_idx=${maxIdxC} top_val=${maxValC.toFixed(4)} class392=${val392C.toFixed(4)}`);
...
(async () => {
    try {
        ...
        console.log("Testing hives.png:");
        await testImage(graphModel, path.resolve(__dirname, '../images/hives.png'));

        // Let's test a few uploads
        const uploadsDir = path.resolve(__dirname, 'uploads/user_uploads');
        const files = fs.readdirSync(uploadsDir).filter(f => f.endsWith('.jpg') || f.endsWith('.png') || f.endsWith('.webp')).slice(0, 10);
